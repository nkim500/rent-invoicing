...
class Lot(SQLModel, table=True):
    ...
    watermeter_id: Optional[int] = Field(
        foreign_key='watermeters.id', sa_type=BigInteger
    )
    inserted_at: datetime = Field(default_factory=et_datetime_now)
    ...
    def check_water(self):
        try:
            ...
            raise ValueError("Lot_id must be constructed as self.property_code + int")
        return self
...
class WaterUsage(SQLModel, table=True):
    ...
    id: UUID = Field(default_factory=uuid4, primary_key=True)
    watermeter_id: int = Field(foreign_key='watermeters.id', nullable=False, sa_type=BigInteger)
